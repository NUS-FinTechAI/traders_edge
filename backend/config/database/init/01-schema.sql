...
CREATE OR REPLACE FUNCTION unlock_first_module_level_after_quiz()
RETURNS TRIGGER AS $$
DECLARE
    v_quiz_type TEXT;
    v_module INTEGER;
...
BEGIN
    ...
    SELECT quiz_type, module
    INTO v_quiz_type, v_module
    FROM module_quizzes
    WHERE quiz_id = NEW.quiz_id;

    IF v_quiz_type <> 'pre' THEN
        RETURN NEW;
    END IF;
...
DROP TRIGGER IF EXISTS trg_unlock_first_module_level_after_quiz
ON user_module_quiz_progress;

CREATE TRIGGER trg_unlock_first_module_level_after_quiz
AFTER INSERT OR UPDATE OF completed
ON user_module_quiz_progress
FOR EACH ROW
EXECUTE FUNCTION unlock_first_module_level_after_quiz();
